// Creating objects - p64

// let cat = {
//     "legs" : 3,
//     "name" : "Harmony",
//     "colour" : "Tortoiseshell"
// };



// Keys without quotes - 65

// console.log(cat);

// let cat = {
//     legs : 3,
//     name : "Harmony",
//     colour : "Tortoiseshell"
// };

// console.log(cat);

// let cat = {
//     legs : 3,
//     "full name" : "Harmony Philomena Snuggly-Pants Morgan",
//     colour : "Tortoiseshell"
// };

// console.log(cat);
// console.log(cat["full name"]);
// console.log(cat.colour);
// console.log(cat.legs);
// console.log(cat["full name"])

// console.log(Object.keys(cat));

// let dog = {
//     name : "Pancake",
//     age 
// }

// Accessing values in objects - p66

// let dog = {name: "Pancake", age: 6, colour: "white", bark: "Yip yap yip!"};
// let cat = {name: "Harmony", age: 8, colour: "tortoiseshel"};
// console.log(Object.keys(dog))
// console.log(Object.keys(cat))


// // Adding values to objects - p67

// cat = {};

// cat["legs"] = 3;
// cat.name = "Dog";
// cat.colour = "Tortoiseshel";
// console.log(cat);




//Adding keys with dot notation - p68

// console.log(DocumentFragment.isBrown);



//Combining arrays and objects - p69

// let dinosaurs = [
//     {name:"Tyranosaurus Rex", period:"Late Cretaceous"},
//     {name:"Stegosaurus", period:"Late Jurassic"},
//     {name:"Plateosaurus", period:"Triassic"}
// ];

// console.log(dinosaurs[0]);
// console.log(dinosaurs[0].name);
// console.log(dinosaurs[1].period);
// console.log(dinosaurs[2].period);



//An array of friends - p69

// let anna = {name:"Anna", age:11, luckyNumbers:[2, 4, 8, 16]};
// let dave = {name:"Dave", age:5, luckyNumbers:[3, 9, 40]};
// let kate = {name:"Kate", age:9, luckyNumbers:[1, 2, 3]};

// let friends = [anna, dave, kate];

// console.log(friends[1].luckyNumbers[2]);
// console.log(friends[2].name)
// console.log(friends[0].luckyNumbers[2]);



//Exploring objects in the console p71




//Useful Things to do with objects - p72

// let owedMoney = {};
// owedMoney["Jimmy"] = 5;
// owedMoney["Anna"] = 7;
// console.log(owedMoney["Jimmy"]);
// console.log(owedMoney["Jinen"]);
// owedMoney["Jimmy"] += 3;
// console.log(owedMoney["Jimmy"]);
// console.log(owedMoney);
// owedMoney["Anna"] -= 2;
// console.log(owedMoney["Anna"]);



//Storing Information about your movies - p74

let movies = {
    "Finding Nemo": {
        releaseDate: 2003,
        duration: 100,
        actors: ["Albert Brooks", "Ellen DeGeneres", "Alexander Gould"],
        format: "DVD"
    },
    "Star Wars: Episode VI - Return of the Jedi": {
        releaseDate: 1983,
        duration: 134,
        actors: ["Mark Hamill", "Harrison Ford", "Carrie Fisher"],
        format: "DVD"
    },
    "Harry Potter and the Goblet of Fire": {
        releaseDate: 2005,
        duration: 157,
        actors: ["Daniel Radcliffe", "Emma Watson", "Rupert Grint"],
        format: "DVD"
    }
};


let findingNemo = movies["Finding Nemo"];
console.log(findingNemo.duration);
console.log(findingNemo.format);

let cars = {
    releaseDate: 2006,
    duration: 177,
    actors: ["Owen Wilson", "Bonnie Hunt", "Paul Newman"],
    format: "Blu-ray"
};
movies["cars"] = cars;

console.log(Object.keys(movies));
